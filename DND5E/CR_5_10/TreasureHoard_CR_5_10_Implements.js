...
if (numMaItems.total > 0) {
    ...
    for (let itemRarity of itemRarities) {
        switch (itemRarity) {
            case 1:
                let table1 = await fromUuid("Compendium.dnd-dungeon-masters-guide.tables.RollTable.dmgImplementsCom");
                let result1 = await table1.roll();
                items.push(result1.results[0].text);
                break;
            case 2:
                let table2 = await fromUuid("Compendium.dnd-dungeon-masters-guide.tables.RollTable.dmgImplementsUnc");
                let result2 = await table2.roll();
                items.push(result2.results[0].text);
                break;
            case 3:
                let table3 = await fromUuid("Compendium.dnd-dungeon-masters-guide.tables.RollTable.dmgImplementsRar");
                let result3 = await table3.roll();
                items.push(result3.results[0].text);
                break;
            case 4:
                let table4 = await fromUuid("Compendium.dnd-dungeon-masters-guide.tables.RollTable.dmgImplementsVer");
